...
class SQLServer(object):
    ...
    def __init__(self, driver=None, server=None, user=None, password=None, dsn=None, database=None):
        ...
        self.conn_values = [driver, server, database, user, password]
    ...
    def connect(self, authentication='sql', autocommit=False):
        ...
        if authentication != 'windows' and authentication != 'sql':
            ...
            uid_pwd_str = ';'.join(['{}={}'.format(obj,val) for obj, val in zip(conn_objs[-2:], self.conn_values[-2:]) if val])
            self.connection_string = '{};{}'.format(dns_str_, uid_pwd_str)
        else:
            self.connection_string = ';'.join(['{}={}'.format(obj,val) for obj, val in zip(conn_objs, self.conn_values) if val])
    ...
    def detail_table(self, table, dataframe=True):
        ...
        data = [{c:v for c,v in zip(columns,tupla)} for tupla in output_data]
        if dataframe:
    ...
    def select(self, table, percent=None, columns=None, condition=None, dataframe=True, schema=None, verbose=True):  
        ...
        if dataframe:
            return pd.DataFrame([{c:v for c,v in zip(df_columns,tupla)} for tupla in data])
        else:
            if len(df_columns) == 1:
                return [v for row in data for v in row]
            else:
                return [{c:v for c,v in zip(df_columns,tupla)} for tupla in data]
